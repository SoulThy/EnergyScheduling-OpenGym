...
"""
Run NO-LEARNING simulations (ONLY_WORKERS) with a temporary worker crash.

Failure model (requested for thesis):
- Crash the most powerful worker at t=4000 s
- Keep it crashed until t=8000 s (duration 4000 s)
- During crash, the worker keeps consuming idle energy (Node._process_energy keeps running)
"""

MODULE = "RunNoLearningFailureOnlyWorkers"
...
def run_simulation(policy: Node.NoLearningPolicy) -> None:
    session_id = f"{SESSION_ID}_{policy.name}_ONLY_WORKERS_FAILURE"
    ...
    # Define worker speeds for this scenario (keep consistent with legacy no-learning runs).
    worker_specs = [
        (1, 1.8, worker_batt_1),
        (2, 1.7, worker_batt_2),
        (3, 1.4, worker_batt_3),
...
if __name__ == "__main__":
    ...
    print("All failure simulations (ONLY_WORKERS) completed.")
